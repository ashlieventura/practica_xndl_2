...
def build_model(cfg, n_classes):
    """ResNet-9 adaptat per a imatges 32×32 en escala de grisos.

    Per què ResNet en lloc de VGG-style?
    El model VGG anterior (86% val acc) s'estancava: train acc pujava
    fins a 90%+ però val acc no superava 86%, senyal clar que els
    gradients no arribaven bé a les primeres capes (vanishing gradient).
    Les skip connections (x + F(x)) permeten que el gradient salti
    directament per la connexió residual, mantenint el flux d'informació
    cap enrere i permetent que les capes inicials continuïn aprenent.

    Estructura (resolució 32×32 → 4×4):
      prep  : Conv 1→64                    32×32  — extreu trets bàsics
      layer1: Conv 64→128 + MaxPool        16×16  + skip (res1)
      layer2: Conv 128→256 + MaxPool        8×8   — sense skip (bloc simple)
      layer3: Conv 256→512 + conv_strided   4×4   + skip (res3)
      cap   : GAP → FC(256) → FC(14)

    Decisions basades en experiments anteriors:
    - CBAM (atenció) eliminat: amb mapes 4×4 el SpatialAttention 7×7
      no aportava res útil i alentegia el forward.
    - Conv strided al bloc final (en lloc de MaxPool): aprèn com fer
      el downsampling en lloc d'usar un màxim fix — ja ho teníem al
      VGG i funcionava bé.
    - GAP en lloc de Flatten: robustesa a posició, menys paràmetres,
      menys overfitting — confirmat als experiments anteriors.
    - dropout_conv baix (0.1) + dropout_fc (0.35): la regularització
      principal ve del weight_decay i l'erasing, no del dropout.
      Valors massa alts frenen les primeres epochs sense benefici net.
    - Sense Mixup: va provar-se però la train acc oscil·lava molt (39-48%)
      i el model convergeix millor sense.
    """
    d_conv = cfg["dropout_conv"]
    d_fc   = cfg["dropout_fc"]

    # Bloc de preparació: extreu trets bàsics sense reduir resolució.
    # Una sola conv en lloc de doble perquè la imatge d'entrada (32×32)
    # ja és petita — no cal comprimir res aquí, volem preservar detalls.
    prep = _conv_bn_relu(1, 64)                         # 1×32×32 → 64×32×32

    # Bloc 1 + skip: duplica canals i fa downsampling.
    # La skip connection suma la sortida del pool amb dues conv addicionals
    # (res1). Per què dues conv a la skip i no una? Amb dues s'aproxima
    # millor la funció identitat residual i es dóna més capacitat al bloc.
    layer1      = _conv_bn_relu(64, 128)                # 64×32×32 → 128×32×32
    layer1_pool = nn.MaxPool2d(2)                       # 128×32×32 → 128×16×16
    res1        = nn.Sequential(
    ...
    # Bloc 2 (sense skip): augmenta la capacitat de representació.
    # No té skip perquè el canvi de canals (128→256) requereix un
    # projection shortcut, que afegeix complexitat. Amb les skip dels
    # blocs 1 i 3, el flux de gradient ja és prou bo sense afegir-ne aquí.
    layer2      = _conv_bn_relu(128, 256)               # 128×16×16 → 256×16×16
    layer2_pool = nn.MaxPool2d(2)                       # 256×16×16 → 256×8×8
    drop2 = nn.Dropout2d(d_conv)

    # Bloc 3 + skip + conv strided: arriba a 512 canals i fa l'últim downsampling.
    # Conv strided (stride=2) en lloc de MaxPool: el model aprèn com reduir
    # la resolució en lloc d'usar un màxim fix. Especialment útil aquí
    # perquè passem de 8×8 a 4×4 — perdre activacions amb MaxPool quan
    # la resolució ja és tan petita pot eliminar informació discriminativa.
    layer3      = _conv_bn_relu(256, 512)               # 256×8×8 → 512×8×8
    layer3_down = nn.Sequential(                        # 512×8×8 → 512×4×4
    ...
    # Cap de classificació: GAP → FC(256) → Dropout → FC(14)
    # GAP elimina la dimensió espacial (512×4×4 → 512), fent el model
    # invariant a la posició exacta dels trets dins del cercle.
    gap     = nn.AdaptiveAvgPool2d(1)
    dropout = nn.Dropout(d_fc)
    fc1     = nn.Linear(512, cfg["fc_sizes"][0])
    fc2     = nn.Linear(cfg["fc_sizes"][0], n_classes)

    class ResNet9(nn.Module):
        def __init__(self):
            super().__init__()
            # Registrem tots els mòduls perquè PyTorch els trobi
            # automàticament per a .parameters(), .to(device), etc.
            self.prep        = prep
            self.layer1      = layer1
        ...
        def forward(self, x):
            ...
            x = self.drop1(x + self.res1(x))          # skip: suma i regularitza
            ...
            x = self.layer3_down(x)                   # 512×8×8 → 512×4×4
            x = self.drop3(x + self.res3(x))          # skip: suma i regularitza

            x = self.gap(x).flatten(1)                # 512×4×4 → 512
            x = self.dropout(x)
            x = torch.relu(self.fc1(x))               # 512 → 256
            return self.fc2(x)                        # 256 → 14
